...
func init() {
	...
	indexCmd.Flags().StringVar(&indexFlags.Engine, "engine", "", "Force extractor engine: ts-morph | tsc-api (optional)")
}
...
func runIndex(_ *cobra.Command, args []string) error {
	...
	resp, err := indexer.RunTSExtract(absRoot, indexer.RunOptions{
		Tsconfigs:    indexFlags.Tsconfig,
		IncludeTests: indexFlags.IncludeTests,
		ForceEngine:  indexFlags.Engine,
	})
	if err != nil {
		if resp != nil {
